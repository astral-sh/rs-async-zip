...
impl ZipEntryBuilder {
    ...
    /// Sets the compressed and uncompressed sizes for the entry.
    ///
    /// For ordinary streaming writes these are only local-header hints. Pre-compressed
    /// writes use the uncompressed size as authoritative metadata.
